...
var TeaSupplyChain = artifacts.require("../contracts/TeaSupplyChain.sol");
...
contract('TeaSupplyChain', function(accounts) {
	...
	beforeEach(async() => {
		...
		this.teaSupplyChain = await TeaSupplyChain.new(this.supplyChainStorage.address,{from: authorizedCaller});
		this.supplyChainUser = await SupplyChainUser.new(this.supplyChainStorage.address,{from: authorizedCaller});

		await this.supplyChainStorage.authorizeCaller(this.teaSupplyChain.address,{from: authorizedCaller});
	...
	{
		var _teaFamily = "Rubiaceae";
		var _typeOfSeed = "Tea Arabica";
	...
	describe("Cultivation Activities",() => {
		...
		it("should add cultivation basic details",async() => {
			...
			const { logs } = addFarmBasicDetails(this.teaSupplyChain);
		...
		it("should get cultivation basic details",async() => {
			...
			const activityData = await this.teaSupplyChain
		...
		it("should update farm inspection details",async() => {
			...
			var { logs } = await addFarmBasicDetails(this.teaSupplyChain);
			...
			var { logs } = await updateFarmInspectorData(this.teaSupplyChain,batchNo);
		...
		it("should get farm inspection details",async() => {
			...
			var { logs } = await this.teaSupplyChain
										.addBasicDetails(
											_registrationNo,
											_farmerName,
										    _farmAddress,
			...
			assert.equal(activityData[0],_teaFamily,"Tea Family Check:");
